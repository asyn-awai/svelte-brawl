<script lang='ts'>
    ...
    onMount(() => {
        savedProfiles = JSON.parse(window.localStorage.getItem("savedProfiles"))
    })
</script>
...
<div class='flex gap-5 items-center'>
    ...
    <div class='flex flex-col gap-5 relative'>
        <div class='flex items-center'>
            <div class='bg-white flex flex-col -skew-x-12 border-4 border-black border-r-0 rounded-tr-none rounded-br-none rounded-md w-44 h-12'>
            ...
            class={`hover:scale-105 transition-all rounded-md rounded-tl-none rounded-bl-none border-4 border-black h-12 w-14 -skew-x-12`}>
                <div class='w-full h-full flex items-center justify-center bg-[#256ffe] p-2 border-y-4 border-t-[#2192fb] border-b-[#1059ec]'>
                    <img class='skew-x-12 aspect-square w-5/6' src="https://brawlstats.com/dist/search.bf6de760d92b0442827326e3324455ff.png" alt="search" />
                </div>
            </button>
        </div>
        <div class={`text-black absolute w-full flex flex-col items-center min-h-24 top-[120%] transition-transform origin-top ${inputFocused ? "scale-y-100" : "scale-y-0"}`}>
